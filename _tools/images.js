...
export function register(ctx) {
  ...
  // Replace Image: the box (x/y/w/h) never moves — the new image gets the
  // largest CENTERED crop window matching the box aspect, i.e. the same
  // visible region the old image occupied. Same aspect → crop: null.
  function computeReplaceCrop(o, newW, newH) {
    const boxRatio = o.w / o.h;
    const newRatio = newW / newH;
    if (!isFinite(boxRatio) || !isFinite(newRatio) || boxRatio <= 0 || newRatio <= 0) return null;
    let cropWPct, cropHPct;
    if (newRatio > boxRatio) { cropHPct = 100; cropWPct = (boxRatio / newRatio) * 100; }
    else { cropWPct = 100; cropHPct = (newRatio / boxRatio) * 100; }
    if (cropWPct >= 99.9 && cropHPct >= 99.9) return null;
    return { x: (100 - cropWPct) / 2, y: (100 - cropHPct) / 2, w: cropWPct, h: cropHPct };
  }
  ...
  ctx.viewportEl.addEventListener('drop', async (e) => {
    ...
    // Land the image where it was DROPPED (stagger any extras from there)
    const center = ctx.screenToWorld(e.clientX, e.clientY);
    ctx.pushUndo();
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!filePath) { ctx.showToast(`Couldn't read the dropped file's path (${file.name})`); continue; }
      const result = await ctx.io.dropImage(filePath);
      if (!result || !result.assetPath) { ctx.showToast(`Couldn't import ${file.name}`); continue; }
  ...
  function openInlineCrop(obj) {
    ...
    // Round handles on the image ghost's corners — drag to SCALE the image
    // proportionally under the fixed frame (zoom the crop).
    for (const pos of ['tl', 'tr', 'bl', 'br']) {
      const h = document.createElement('div');
      h.className = `img-scale-handle ish-${pos}`;
      h.dataset.scaleHandle = pos;
      wrapper.appendChild(h);
    }
    const scaleHandles = wrapper.querySelectorAll('.img-scale-handle');
    ...
    function updateCrop() {
      ...
      scaleHandles.forEach(h => {
        const p = h.dataset.scaleHandle;
        h.style.left = ((p.includes('l') ? S.ix : S.ix + S.fullW) - 6) + 'px';
        h.style.top = ((p.includes('t') ? S.iy : S.iy + S.fullH) - 6) + 'px';
      });
    }
    updateCrop();
    ...
    wrapper.addEventListener('mousedown', (ev) => {
      if (ev.target.closest('.crop-handle') || ev.target.closest('.img-scale-handle') || ev.button !== 0) return;
    ...
    // Round handles on the image ghost: SCALE the image proportionally,
    // anchored at the opposite corner — zooming the picture under the
    // fixed frame. Clamped so the image never shrinks below the frame.
    scaleHandles.forEach(h => {
      h.addEventListener('mousedown', (ev) => {
        if (ev.button !== 0) return;
        ev.preventDefault(); ev.stopPropagation();
        const sx = ev.clientX, sy = ev.clientY;
        const oix = S.ix, oiy = S.iy, ofw = S.fullW, ofh = S.fullH;
        const pos = h.dataset.scaleHandle;
        const ax = pos.includes('l') ? oix + ofw : oix; // anchor = opposite corner
        const ay = pos.includes('t') ? oiy + ofh : oiy;
        const d0 = Math.hypot(ofw, ofh);
        const sMin = Math.max(S.rw / ofw, S.rh / ofh); // keep the frame covered
        const zoom = ctx.getZoom();
        function onMove(mv) {
          const dx = (mv.clientX - sx) / zoom, dy = (mv.clientY - sy) / zoom;
          const sdx = pos.includes('r') ? dx : -dx;
          const sdy = pos.includes('b') ? dy : -dy;
          const s = Math.max(sMin, Math.hypot(Math.max(1, ofw + sdx), Math.max(1, ofh + sdy)) / d0);
          S.fullW = ofw * s;
          S.fullH = ofh * s;
          S.ix = pos.includes('l') ? ax - S.fullW : ax;
          S.iy = pos.includes('t') ? ay - S.fullH : ay;
          clampImage();
          updateCrop();
        }
        function onUp() { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); }
  ...
  return {
    ...
    css: `
      ...
      /* Round handles on the image GHOST's corners: scale the image
         proportionally under the fixed frame (square handles = frame). */
      .img-scale-handle {
        position: absolute;
        width: 12px;
        height: 12px;
      ...
      .img-scale-handle.ish-tl, .img-scale-handle.ish-br { cursor: nwse-resize; }
      .img-scale-handle.ish-tr, .img-scale-handle.ish-bl { cursor: nesw-resize; }
    ...
    objectTypes: {
      image: {
        ...
        async onPaste(obj) {
          ...
          try {
            // ctx.pasteSourceProject: relative paths resolve against the
            // folder the clipboard came from
            const res = await ctx.io.importExternalAsset(obj.content, ctx.pasteSourceProject);
        ...
        menu: () => [
          ...
          {
            label: 'Replace Image…',
            icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>',
            async action(ctx) {
              ctx.closeMenus();
              const results = await ctx.io.importImages({ hiRes: false });
              if (!results || !results.length || !results[0] || results[0].error || !results[0].assetPath) return;
              const r = results[0];
              ctx.pushUndo();
              let count = 0;
              for (const sid of ctx.selectedIds) {
                const o = ctx.findObject(sid);
                if (!o || o.type !== 'image') continue;
                o.content = r.assetPath;
                o.crop = computeReplaceCrop(o, r.width, r.height);
                count++;
              }
              if (count) { ctx.renderObjects(); ctx.markDirty(); }
            },
          },
        ],
      },
