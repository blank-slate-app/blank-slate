...
export function register(ctx) {
  ...
  // Deck preview pages render this wide (the panel card is ~350px, so
  // 800px is crisp at 2x and keeps the flip-through PDF tiny)
  const DECK_PREVIEW_W = 800;
  ...
  // Background fill for every selected artboard.
  // null = clear (canvas shows through; export = dark + grid); else solid.
  function setBg(bg) {
    ctx.closeMenus();
    ctx.pushUndo();
    for (const sid of ctx.selectedIds) {
      const o = ctx.findObject(sid);
      if (o && o.type === 'artboard') o.artboardBg = bg;
    }
    ctx.renderObjects();
    ctx.markDirty();
  }
  ...
  function renderArtboardToDataUrl(ab, opts) {
    // preview: small DECK_PREVIEW_W-wide pages for the flip-through PDF;
    // default: full 300-DPI export
    const preview = !!(opts && opts.preview);
    const base = ARTBOARD_EXPORT_SIZES[ab.artboardRatio] || ARTBOARD_EXPORT_SIZES['1:1'];
    const quality = preview ? 0.65 : 0.92;
    const exportW = preview ? DECK_PREVIEW_W : base.w;
    const exportH = preview ? Math.round(base.h * (DECK_PREVIEW_W / base.w)) : base.h;
    ...
    if (ab.artboardBg) {
      // Solid background fill (White / Orange 25%) — no grid
      c2d.fillStyle = ab.artboardBg;
      c2d.fillRect(0, 0, exportW, exportH);
    } else {
    // Clear: dark desk background (match the page colour #111111) + grid
  ...
  // PUBLISH: build the tidy shareable deck folder (the artboards tool owns
  // this because the deck IS the artboards — it renders the preview pages
  // and picks the clean object set). Rules: only objects overlapping an
  // artboard ship; images/tools/PDF handled by the publish-deck io call.
  async function publishDeck() {
    ...
    if (artboards.length === 0) { ctx.showToast('Add at least one artboard to publish a deck'); return; }

    // Clean object set: artboards + everything overlapping one of them
    const overlapsAny = (o) => artboards.some(ab =>
      o.x + o.w > ab.x && o.x < ab.x + ab.w && o.y + o.h > ab.y && o.y < ab.y + ab.h);
    const clean = ctx.objects.filter(o => o.type === 'artboard' || overlapsAny(o));

    // Small preview pages (DECK_PREVIEW_W wide) keep the PDF tiny
    const pages = artboards.map((ab, i) => {
      const base = ARTBOARD_EXPORT_SIZES[ab.artboardRatio] || ARTBOARD_EXPORT_SIZES['1:1'];
      const w = DECK_PREVIEW_W;
      const h = Math.round(base.h * (DECK_PREVIEW_W / base.w));
      return { name: `page-${i + 1}`, w, h, dataUrl: renderArtboardToDataUrl(ab, { preview: true }) };
    });

    const author = 'Forma Rosa Creative'; // TODO: user profile when accounts exist
    const res = await ctx.io.publishDeck({
      title: ctx.project,
      author,
      objects: JSON.parse(JSON.stringify(clean)),
      canvasState: { panX: 0, panY: 0, zoom: 1, toolState: {} },
      pages,
    });
    if (!res || res.canceled) return;
    if (res.error) { ctx.showToast(res.error); return; }
    ctx.showToast(`Deck published: ${pages.length} page${pages.length === 1 ? '' : 's'}, ${res.images} image${res.images === 1 ? '' : 's'}, ${res.tools} unique tool${res.tools === 1 ? '' : 's'}`);
  }
  ...
  return {
    ...
    objectTypes: {
      artboard: {
        defaults: {
          ...
          artboardFields: null,
          artboardBg: null, // null = clear · '#FFFFFF' · '#F0C9B4' (orange 25%)
        },

        // Deck pages don't spin — 'r' skips artboards entirely
        rotatable: false,

        normalize(obj) {
          obj.zIndex = 0; // artboards always sit behind everything
          if (!ARTBOARD_SIZES[obj.artboardRatio]) obj.artboardRatio = '1:1';
          if (!['#FFFFFF', '#F0C9B4'].includes(obj.artboardBg)) obj.artboardBg = null;
          obj.rotation = 0;
          sanitizeFields(obj);
        },
        ...
        async onPaste(obj) {
          ...
          for (const corner of AB_CORNERS) {
            ...
            if (f && f.kind === 'logo' && f.src) {
              try {
                const res = await ctx.io.importExternalAsset(f.src, ctx.pasteSourceProject);
        ...
        render(obj, el) {
          ...
          if (obj.artboardBg) el.style.background = obj.artboardBg; // solid fill; null = clear
        ...
        // ── MENUS ── artboard section: Proportion ▶, Background ▶, Export,
        // Renumber, Match Last
        menu: (selObjs) => {
          const ab = (selObjs || []).find(o => o.type === 'artboard');
          const curBg = (ab && ab.artboardBg) || null;
          return [
          {
            label: 'Proportion',
          ...
          {
            label: 'Background',
            icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 010 18z" fill="currentColor" stroke="none"/></svg>',
            submenu: [
              { label: 'Clear', checked: curBg === null, action() { setBg(null); } },
              { label: 'White', checked: curBg === '#FFFFFF', action() { setBg('#FFFFFF'); } },
              { label: 'Orange 25%', checked: curBg === '#F0C9B4', action() { setBg('#F0C9B4'); } },
            ],
          },
          {
    ...
    canvasMenu: [
      ...
      {
        label: 'Publish Deck…',
        icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v7a2 2 0 002 2h12a2 2 0 002-2v-7"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>',
        order: 43,
        action(ctx) { publishDeck(); },
      },
      {
